...
is replaced with a 'sleeping' thread by the operating system scheduller. If
these two threads belong to seperate processes, this can be a relatively
expensive operation, as processes operate with entirely seperate memory spaces.
The operating system therefore needs to:
\begin{itemize}
  \item Select the next thread to occupy the CPU from the queue of waiting
  threads, using a thread-schedulling algorithm (For example: \gls{fifo},
  \gls{lru})
...
Go moves the schedulling of Goroutines out of the operating system kernel, and
...
environment variable}, and chooses which Goroutine to execute next independantly
of the \gls{os}. This allows the Go runtime scheduller to cooperatively schedule
...
data. The fact that the scheduller behaviour is both deterministic, and
